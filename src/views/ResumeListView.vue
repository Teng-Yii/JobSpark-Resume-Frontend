<template>
  <div class="resume-list-container">
    ...
    <header class="header">
      ...
      <el-button type="success" size="large" @click="handleExcellentResume" class="excellent-btn">
    ...
    <el-tabs v-model="activeTab" class="resume-tabs" @tab-change="handleTabChange">
      <el-tab-pane label="我的简历" name="upload">
          ...
          <div v-if="resumeList && resumeList.length > 0" class="resume-grid">
            <el-card v-for="resume in resumeList" :key="resume.resumeId" class="resume-card" shadow="hover">
              <div class="card-header">
                ...
                <div class="actions">
                  <el-button type="primary" plain size="small" @click="handleViewDetail(resume.resumeId)">
                    查看详情
                  </el-button>
      ...
      <el-tab-pane label="优秀简历模板" name="excellent">
        <div v-loading="loading" class="list-content">
          <div v-if="excellentResumeList && excellentResumeList.length > 0" class="resume-grid">
            <el-card v-for="resume in excellentResumeList" :key="resume.resumeId" class="resume-card" shadow="hover">
              <div class="card-header">
                <div class="user-info">
                  <el-avatar :size="50" :src="resume.avatarUrl || defaultAvatar" icon="UserFilled" />
                  <div class="info-text">
          ...
          <el-empty v-else description="暂无优秀简历模板" :image-size="200">
            <el-button type="primary" @click="handleExcellentResume">录入优秀模板</el-button>
          </el-empty>
        </div>
      </el-tab-pane>
    </el-tabs>
...
import { Plus, UserFilled, SwitchButton, Star } from '@element-plus/icons-vue'
import { ElMessage, ElMessageBox } from 'element-plus'
import type { ResumeDetailResponse } from '@/api/resume'
import { getResumeList, getResume } from '@/api/resume'
...
const activeTab = ref<'upload' | 'excellent'>('upload')
const excellentResumeList = ref<ResumeDetailResponse[]>([])

const resumeList = computed(() => resumeStore.resumeList)

const fetchResumes = async (cvType?: string) => {
  if (!userStore.token) {
    router.push('/login')
    return
  }
  ...
  try {
    if (cvType === 'excellent') {
      // 获取优秀简历模板列表
      const response = await getResumeList('excellent')
      excellentResumeList.value = response
    } else {
      // 获取个人简历列表
      await resumeStore.fetchResumeList('upload')
    }
  } catch (error) {
...
const handleTabChange = (tabName: 'upload' | 'excellent') => {
  activeTab.value = tabName
  fetchResumes(tabName)
}
...
const handleViewDetail = async (resumeId: string) => {
  try {
    ...
    // 重定向到简历详情页面（只查看，不优化）
    router.push(`/resume/detail?id=${resumeId}`)
  } catch (error) {
